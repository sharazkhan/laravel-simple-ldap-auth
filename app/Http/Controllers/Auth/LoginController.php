...
use AuthenticatesUsers;
    ...
    public function __construct() {
        //echo config('adldap_auth.usernames.eloquent'); exit;
    ...
    protected function attemptLogin(Request $request) {
        ...
        //$userdn = sprintf($username);
        $userdn = $username;
        // $userdn = $username . '@northamerica.cerner.net';
        //echo $userdn; exit;
        //if (Adldap::auth()->attempt($userdn, $password, $bindAsUser = true)) {
        if (Adldap::getProvider('default')->auth()->attempt($userdn, $password, $bindAsUser = true)) {
    ...
    protected function retrieveSyncAttributes($username) {
        // echo $username; exit;
        //echo env('ADLDAP_USER_ATTRIBUTE'), '=', $username; exit;
        // $ldapuser = Adldap::search()->where(env('ADLDAP_USER_ATTRIBUTE'), '=', $username)->first();
        $ldapuser = Adldap::getProvider('default')->search()->where(env('ADLDAP_USER_ATTRIBUTE'), '=', $username)->first();
        // dd($ldapuser);
        // echo "<pre>"; print_r($ldapuser);  echo "</pre>"; exit;
        //$ldapuser = Adldap::getProvider('default')->search()->users()->get();
        if (!$ldapuser) {
            // log error
            return false;
        }
